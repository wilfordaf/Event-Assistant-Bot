...
class OllamaGenerationModel(ITagGenerationModel):
    ...
    def generate_tags(self, text: str, relevant_skill_tags: List[str]) -> List[str]:
        self._logger.debug(f"Generating tags for {text}")

        self._possible_tags = list(set(self._possible_tags) | set(relevant_skill_tags))
        prompt = self._prompt.format(**{"text": text, "possible_tags": self._possible_tags})

        try:
            response: str = requests.post(
                self._generate_connection_string,
                json={**self._PARAMS, "prompt": prompt, "options": self._OPTIONS},
