...
from src.service.entities.api_models.input.add_event_request_body import AddEventRequestBody
from src.service.models.rag_information import ZeroShotRagTagsRetriever
...
class ServiceAssembler:
    ...
    def __init__(self):
        ...
        self._rag_model = ZeroShotRagTagsRetriever()
        self._rag_model.load_model()
    ...
    def get_add_tags_by_text_to_user_response(
        ...
        cv_class = self._rag_model.classify_cv(request_body.text)
        relevant_rags = self._rag_model.retrieve_tags_by_class(cv_class)
        generated_tags = self._tag_generation_model.generate_tags(request_body.text, relevant_rags)
    ...
    def get_add_tags_by_link_to_user_response(
        ...
        text = retrieve_linkedin_data(request_body.link)
        cv_class = self._rag_model.classify_cv(text)
        relevant_rags = self._rag_model.retrieve_tags_by_class(cv_class)
        generated_tags = self._tag_generation_model.generate_tags(text, relevant_rags)
        response = self._add_tags_to_user(request_body.telegram_id, generated_tags)
    ...
    def get_add_tags_by_cv_to_user_response(
        ...
        text = retrieve_pdf_data(request_body.text)
        cv_class = self._rag_model.classify_cv(text)
        relevant_rags = self._rag_model.retrieve_tags_by_class(cv_class)
        generated_tags = self._tag_generation_model.generate_tags(text, relevant_rags)
        response = self._add_tags_to_user(request_body.telegram_id, generated_tags)
    ...
    def get_add_tags_by_dialogue_to_user_response(
        ...
        text = retrieve_json_data(request_body.text, request_body.telegram_id)
        cv_class = self._rag_model.classify_cv(text)
        relevant_rags = self._rag_model.retrieve_tags_by_class(cv_class)
        generated_tags = self._tag_generation_model.generate_tags(text, relevant_rags)
        response = self._add_tags_to_user(request_body.telegram_id, generated_tags)
