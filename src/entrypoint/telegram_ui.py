...
from src.presentation.button_texts import (
    add_data_text,
    add_link_to_profile_text,
    add_person_description_text,
    back_text,
    cancel_text,
    create_event_text,
    delete_data_text,
    delete_profile_from_event_text,
    dislike_text,
    find_match_text,
    go_to_admin_page_text,
    go_to_main_page_text,
    go_to_save_profiles_text,
    join_event_text,
    like_text,
    list_of_participants_text,
    no_text,
    open_event_list_text,
    open_my_tags_list_text,
    saved_users_list_text,
    start_dialog_text,
    try_again_text,
    upload_cv_text,
    upload_dialog_text,
...
from src.presentation.message_texts import (
    admin_panel_message_text,
    description_was_saved,
    enter_event_name_message_text,
    event_not_found_message_text,
    event_page_with_empty_profile_message_text,
    event_was_created_message_text,
    function_is_not_available,
    handler_not_found_message_text,
    link_was_added,
    list_participants_of_event_message_text,
    open_main_page_message_text,
    request_confirmation_messaget_text,
    request_description_message_text,
    request_event_key_message_text,
    request_link_message_text,
    saved_profiles_doesnt_exist,
    select_data_type_message_text,
    select_file_message_text,
    uncorrect_command_message_text,
    uncorrect_link_message_text,
...
def show_my_tags(message):
    try:
        ...
        tags = service.get_tags_by_user_response(request_body)["body"]["tags"]  # type: ignore
...
def create_event_handler(message):
    ...
    try:
        ...
        request_body = AddUserToEventRequestBody.model_validate(
            {"telegram_id": message.from_user.username, "title": event_name}  # type: ignore
        )
        service.get_add_user_to_event_response(request_body)["body"]["success"]  # type: ignore
...
def join_event_handler(message):
    if message.text == go_to_main_page_text:
        ...
        success = service.get_add_user_to_event_response(request_body)["body"]["success"]  # type: ignore
        if success:
...
def handle_admin_panel(message):
    ...
    participants = service.get_users_by_event_response(request_body)["body"]["users"]  # type: ignore
...
def open_event_page(message, has_profile_in_event=True):
    ...
    is_admin = service.get_is_admin_response(request_body)["body"]["success"]  # type: ignore
...
def open_match_list(message):
    ...
    candidates = service.get_user_ranking_response(request_body)["body"]["users"]  # type: ignore
    print(candidates)
